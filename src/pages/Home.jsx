  ...
  return (
    <>
      ...
      >
        ...
        {/* 2 — Album + tracklist */}
        <Panel width="min(1150px, 94vw)" className="group/album px-8 md:px-16">
          <BackdropLoop src="/video/bg-1.mp4" opacity={0.1} />
          ...
          <div className="relative grid gap-10 md:grid-cols-[minmax(0,320px)_1fr] md:gap-16">
            <motion.div
              initial={{ opacity: 0, scale: 0.94 }}
              whileInView={{ opacity: 1, scale: 1 }}
              viewport={{ once: true }}
              transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }}
              className="corners self-center"
            >
              <img src={album.cover} alt={album.title} className="w-full" />
              <p className="h-display mt-5 text-3xl">{album.title}</p>
              ...
              {tracklist.map((tr, i) => (
                ...
                >
                  <button
                    ...
                    className="group flex w-full items-center gap-4 border-b border-white/10 py-2.5 text-left transition-colors duration-300 hover:border-acid/60 md:gap-6 md:py-3"
                  >
                    <span className="w-7 shrink-0 text-xs text-white/30 transition-colors duration-300 group-hover:text-acid">
                      {tr.number}
                    </span>
                    <span className="h-display flex-1 truncate text-lg transition-colors duration-300 group-hover:text-acid md:text-2xl">
                      {tr.title}
                      {tr.feat && (
        ...
        <Panel width="auto" className="px-8 md:px-16">
          <BackdropLoop src="/video/bg-2.mp4" opacity={0.08} />
        ...
        {/* 4 — Clips */}
        <Panel width="auto">
          <BackdropLoop src="/video/bg-3.mp4" opacity={0.09} />
          <ClipsPanel onPlay={setPlaying} />
        </Panel>

        {/* 5 — Boutique */}
        <Panel width="auto" className="px-8 md:px-16">
          <BackdropLoop src="/video/bg-4.mp4" opacity={0.08} />
          <div className="relative flex items-center gap-6 md:gap-8">
            <div className="mr-4 shrink-0">
        ...
        {/* 6 — Sortie */}
        <Panel width="min(1000px, 96vw)" className="px-8 md:px-16">
          {/* Boucle de fond conservée, l'animation Aetherstone se pose dessus. */}
          <BackdropLoop src="/video/bg-3.mp4" opacity={0.22} />
          <ScrubTrack />
          <div
            aria-hidden
          ...
          >
            <div className="text-center md:text-left">
              ...
              <motion.img
                src="/img/logo-partylife-mark.png"
                alt="PARTY LIFE"
